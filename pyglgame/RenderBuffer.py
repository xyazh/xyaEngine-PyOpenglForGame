...
class RenderBuffer:
    ...
    def build(self):
        ...
        glBufferData(GL_ARRAY_BUFFER, len(vertices) * sizeof(c_float),(c_float * len(vertices))(*vertices), self.usage)

    def draw(self,use_dis_shader:bool=True):
        self.build()
        if use_dis_shader:
            RenderGlobal.instance.dis_shader.use()
            RenderGlobal.instance.dis_shader.uniform1i("formatType",self.buffer_builder.format_type)
            glBindBuffer(GL_ARRAY_BUFFER, self.vbo)
